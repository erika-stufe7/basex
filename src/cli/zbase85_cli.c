...
static void print_usage(const char* progname) {
    ...
    printf("Compress with zstd and encode with Base85, or decode and decompress FILE, or standard input, to standard output.\n\n");
    ...
    printf("Pipeline: INPUT -> zstd compress (level %d) -> base85 encode -> OUTPUT\n", ZSTD_DEFAULT_LEVEL);
    printf("Report bugs to: https://github.com/yourusername/basex\n");
}

static void print_version(void) {
    printf("zbase85 (BaseX) %s with zstd %s\n", basex_version(), ZSTD_versionString());
    printf("Fast zstd compression with Base85 encoding and CPU optimizations\n");
}
...
int main(int argc, char* argv[]) {
    ...
    if (verbose && !quiet) {
        fprintf(stderr, "zbase85: zstd v%s, compression level %d, threads %d\n", 
    ...
    uint8_t* base85_buffer = malloc(BUFFER_SIZE * 2);
    uint8_t* filtered_buffer = malloc(BUFFER_SIZE * 2);
    
    if (!in_buffer || !zstd_buffer || !base85_buffer || !filtered_buffer) {
    ...
    while (1) {
        ...
        if (decode) {
            ...
            // Base85 decode
            ssize_t decoded_len = basex_base85_decode((char*)filtered_buffer, filtered_len, zstd_buffer);
            if (decoded_len < 0) {
            ...
            // zstd decompress
            size_t decompressed_size = ZSTD_decompressDCtx(dctx, base85_buffer, BUFFER_SIZE * 2, 
            ...
            fwrite(base85_buffer, 1, decompressed_size, stdout);
            total_out += decompressed_size;
            
        } else {
            // Encode: zstd compress -> Base85
            ...
            // Base85 encode
            ssize_t encoded_len = basex_base85_encode(zstd_buffer, compressed_size, (char*)base85_buffer);
            if (encoded_len < 0) {
            ...
            if (wrap_cols > 0) {
                ...
                fwrite(base85_buffer, 1, encoded_len, stdout);
            }
